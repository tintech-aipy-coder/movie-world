...
// Generates a random secret suitable for Telegram's setWebhook `secret_token`
// field. Telegram requires this to only contain A-Z, a-z, 0-9, "_" and "-",
// 1–256 chars — we produce 48 of them (well above what's needed to be
// unguessable). This only RETURNS a fresh value; it can't write it into
// env.WEBHOOK_SECRET for you (Workers can't self-modify their own secrets),
// so it still has to be applied with `wrangler secret put WEBHOOK_SECRET`
// and then /setup-webhook called again so Telegram starts using it.
...
// Edits a message that only exists as `inline_message_id` (i.e. a message
// the bot posted via inline mode, `@bot query`, into a chat the bot itself
// isn't a member of). For these there's no chat_id/message_id — editing in
// place is the ONLY thing the Bot API allows, sendMessage is impossible.
// We don't know up front whether the original inline result was a photo
// (has a caption) or a text article (has a body), so we try
// editMessageCaption first and fall back to editMessageText if Telegram
// rejects it (e.g. "message has no caption to edit").
async function tgEditInline(env, inlineMessageId, text) {
  const base = { inline_message_id: inlineMessageId, parse_mode: "HTML" };
  const capResp = await fetch(`${TG(env[BOT_TOKEN])}/editMessageCaption`, {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({ ...base, caption: text }),
  });
  const capJson = await capResp.json().catch(() => null);
  if (capJson?.ok) return capJson;

  return fetch(`${TG(env[BOT_TOKEN])}/editMessageText`, {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({ ...base, text, disable_web_page_preview: true }),
...
async function onCallback(env, cb, origin) {
  ...
  // Callback came from a message the bot posted via inline mode (the small
  // "📋 جزئیات کامل" result under an "@bot query" search, possibly in a
  // chat/group the bot isn't even a member of). Telegram gives us
  // inline_message_id instead of chat/message ids for these, so tgSend/
  // tgEdit (which need a chat_id) can't be used — editing that same inline
  // message in place is the only option the Bot API offers.
  if (!chatId) {
    if (inlineMessageId && data.startsWith("m:")) {
      const [, type, id] = data.split(":");
      await tgAnswerCb(env, cb.id, "در حال بارگذاری جزئیات...");
      await sendMovieDetailsInline(env, inlineMessageId, id, type);
    }
    return;
  }
...
// Inline-mode counterpart of sendMovieDetails. There's no chat to push
// separate messages into (see the inline_message_id note in onCallback), so
// everything — full details + subtitle links — is merged into ONE block and
// written into the existing inline message via tgEditInline.
async function sendMovieDetailsInline(env, inlineMessageId, id, type = "movie") {
  const raw = await getFullDetails(id, env, type);
  if (!raw) {
    await tgEditInline(env, inlineMessageId, "⚠️ خطا در دریافت جزئیات.");
    return;
  }
  const subs = await findSubtitles(raw.title, raw.year);
  const combined = `${fmtDetails(raw)}\n\n${fmtSubtitleLinks(subs)}`;
  await tgEditInline(env, inlineMessageId, combined);
}
  ...
  async fetch(request, env, ctx) {
    ...
    if (url.pathname === "/webhookga") {
      ...
      return new Response(JSON.stringify({
        ...
        note:
          "این کلید فقط تولید شده، خودکار جایگزین نشده. برای اعمالش: " +
          "1) wrangler secret put WEBHOOK_SECRET (و همین مقدار رو پیست کن) " +
          "2) دوباره /setup-webhook رو صدا بزن تا تلگرام کلید جدید رو ثبت کنه.",
      }, null, 2), {
    ...
    // Admin-only helper: generates a fresh random value to use as
    // WEBHOOK_SECRET. A Worker can't write its own secrets at runtime, so
    // this endpoint doesn't set anything by itself — it just hands you a
    // strong random string to paste into:
    //   wrangler secret put WEBHOOK_SECRET
    // After that, call /setup-webhook again so Telegram picks up the new
    // secret_token (old one stops being accepted on /webhook the moment the
    // new WEBHOOK_SECRET is deployed).
    if (url.pathname === "/generate-webhook-secret") {
      if (!isAdminAuthorized(request, env, url)) return new Response("Unauthorized", { status: 401 });
      ...
      return new Response(JSON.stringify({
        new_webhook_secret: newSecret,
        next_steps: [
          `wrangler secret put ${WEBHOOK_SECRET_ENV}`,
          "(paste the new_webhook_secret value above when prompted)",
          "wrangler deploy",
          "then call /setup-webhook again to re-register it with Telegram",
        ],
      }, null, 2), {
        headers: { "Content-Type": "application/json" },
      });
